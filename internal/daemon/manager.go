...
func newPipelineAgent(ctx context.Context, cfg *config.Config, evidenceRoot string, lookPath func(string) (string, error), environments ...runenv.Overlay) (agent.Agent, error) {
	...
	primaryConfig := cfg
	if _, explicitFixer := cfg.ReviewAgents["fixer"]; explicitFixer {
		// An explicit upstream role replaces the legacy fixer, including its
		// launcher. Do not resolve or create a fixer that will never be used.
		withoutLegacyFixer := *cfg
		withoutLegacyFixer.ReviewFixerAgent = ""
		withoutLegacyFixer.ReviewFixerCommand = nil
		primaryConfig = &withoutLegacyFixer
	}
	primary, err := newConfiguredAgent(ctx, primaryConfig, evidenceRoot, lookPath, environment)
	if err != nil {
		return nil, err
	}
...
func (m *RunManager) startRunWithIntentSourceLocked(ctx context.Context, repo *db.Repo, branch, headSHA, baseSHA, trigger string, skipSteps []types.StepName, intent, source, launchNonce, validationGeneration, intentDigest, prBaseBranch, inheritedPRURL string) (string, error) {
	...
	// Create default and review-role agents as one owned set.
	ag, err := newPipelineAgent(ctx, cfg, m.paths.EvidenceRoot(cfg.Test.Evidence.LocalRoot), exec.LookPath, forgeEnvironment(forgeCtx))
	if err != nil {
		m.db.UpdateRunError(run.ID, err.Error())
