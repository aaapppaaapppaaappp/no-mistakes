...
func TestPipelineReviewRolesPreserveLegacyFixerCompatibility(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("legacy Codex process fixture uses POSIX shell")
	}
	for _, explicitFixer := range []bool{false, true} {
		t.Run(fmt.Sprintf("explicit_fixer=%t", explicitFixer), func(t *testing.T) {
			dir := t.TempDir()
			capture := filepath.Join(dir, "argv.log")
			piBin := writeCapturingPiAgent(t, dir, capture)
			codexBin := filepath.Join(dir, "codex")
			writeExecutableFixture(t, codexBin, "#!/bin/sh\nprintf '%s\\n' \"$*\" >> "+shellQuoteForTest(capture)+"\n"+`
cat >/dev/null
printf '%s\n' '{"type":"thread.started","thread_id":"legacy-fixer-session"}'
...
`)
			...
			wantRoles := 1
			if explicitFixer {
				// Replacing the legacy fixer must not require its old launcher.
				legacyCommand = fmt.Sprintf("review_fixer_command: [%q]\n", filepath.Join(dir, "removed-launcher"))
				fixerRole = "  fixer: {agent: pi, model: new-fixer, effort: max}\n"
				wantRoles = 2
			}
			global, err := config.LoadGlobalFromBytes([]byte(fmt.Sprintf(`agent: pi
agent_path_override:
  pi: %q
  codex: %q
agent_config:
  pi: {model: default-model, effort: high}
  codex: {model: legacy-fixer, effort: high}
review_fixer_agent: codex
%sreview_agents:
  reviewer: {agent: pi, model: new-reviewer, effort: max}
%s`, piBin, codexBin, legacyCommand, fixerRole)))
			if err != nil {
				t.Fatal(err)
			}
			cfg := config.Merge(global, &config.RepoConfig{})
			if err := cfg.ResolveAgent(context.Background(), exec.LookPath); err != nil {
				t.Fatalf("resolve mixed configuration: %v", err)
			}
			cfg.DisableProjectSettings = true
			ag, err := newPipelineAgent(context.Background(), cfg, t.TempDir(), exec.LookPath)
			if err != nil {
				t.Fatal(err)
			}
			defer ag.Close()
			fixerModel, fixerProvider := "legacy-fixer", "codex"
			if explicitFixer {
				fixerModel, fixerProvider = "new-fixer", "pi"
			}
			for _, tc := range []struct{ purpose, model string }{
				{"review", "new-reviewer"},
				{"review-fix", fixerModel},
				{"review", "new-reviewer"},
				{"test-evidence", "default-model"},
			} {
				if err := os.WriteFile(capture, nil, 0o600); err != nil {
					t.Fatal(err)
				}
				if _, err := ag.Run(context.Background(), agent.RunOpts{Purpose: tc.purpose, Prompt: "hello", CWD: dir}); err != nil {
					t.Fatalf("%s: %v", tc.purpose, err)
				}
				argv, err := os.ReadFile(capture)
				if err != nil {
					t.Fatal(err)
				}
				if !strings.Contains(string(argv), tc.model) || strings.Count(string(argv), "\n") != 1 {
					t.Fatalf("%s must launch exactly one %s process, got %q", tc.purpose, tc.model, argv)
				}
			}
			if !agent.SupportsSessionProvider(ag, fixerProvider) {
				t.Fatalf("effective fixer cannot recover %s sessions", fixerProvider)
			}
			if cfg.ReviewFixerAgent != types.AgentCodex || len(cfg.ReviewAgents) != wantRoles {
				t.Fatal("agent construction mutated the operator's routing configuration")
			}
		})
	}
}
