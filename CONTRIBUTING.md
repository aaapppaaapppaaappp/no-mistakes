...
- One exemption exists, for upstream synchronization: a pull request whose head branch is exactly `fm/no-mistakes-upstream-sync` is exempt from `PR must be raised via no-mistakes` and `Generated files must not be hand-edited` only. A fork-upstream merge is not pipeline-authored and legitimately carries upstream's generated `CHANGELOG.md` and manifest. No other branch and no other check is affected, and that branch is still judged by all of them. See [the action README](.github/actions/require-no-mistakes/README.md).
