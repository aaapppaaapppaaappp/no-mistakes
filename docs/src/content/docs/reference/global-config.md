...
The wrapper disables discovered Pi extensions and explicitly loads only the no-mistakes gate extension for this ACP target. Normal model and authentication configuration remains available, so credentials are not copied, but models that depend on provider extensions are unsupported in this dedicated mode. Keep the trusted source checkout and its installed dependencies at that absolute path. Repeat the pinned install after updating the checkout's lockfile. The [environment reference](/no-mistakes/reference/environment/#no_mistakes_pi_structured_output) owns the opt-in variable's semantics. A valid gate reports an actionable extension error when the runtime dependencies are missing. The existing opted-in target keeps its established extraction and retry behavior. A target that additionally selects the explicit single-attempt contract below instead fails closed unless exactly one completed, exclusive `structured_output` result survives through the end of the turn. Native tool arguments are the sole result. Assistant prose from that attempt is discarded rather than parsed, streamed, or allowed to supplement the arguments, and its presence emits only a bounded warning. Prose without a valid call, zero or multiple calls, another tool, invalid terminal activity, failed/incomplete calls, malformed events, and final schema mismatch return a protocol error and no result.

Flash-Next has one narrower, inactive-by-default Responses example under `integrations/pi/fixtures`. Its separately named `pi-flash-next-responses-gate` target uses `pi-no-mistakes-flash-next-responses-acp`, provider `no-mistakes-flash-next-responses`, model `Qwen/Qwen3.8-Flash-Next-FP8`, `openai-responses`, `xhigh`, required strict tool selection, parallel calls disabled, and the exact sampling values in the model fixture. The wrapper exposes no built-in tools or project settings and creates an owner-only temporary Pi settings directory with every Pi retry layer disabled; it symlinks, rather than copies or edits, the operator's model catalogue and authentication file. The route also sets [`NO_MISTAKES_ACPX_ATTEMPTS=1`](/no-mistakes/reference/environment/#no_mistakes_acpx_attempts), which explicitly adds `--prompt-retries 0` and disables no-mistakes' outer retries for that invocation only. One ACP/Pi session may continue with at most two fixed, trusted format-repair nudges (three provider requests total) for a missing call or recoverable schema/field defect. The nudges carry no assistant content; wrong or competing tools, route drift, competing activity, provider/transport failures, and exhaustion terminate without a cold retry.
