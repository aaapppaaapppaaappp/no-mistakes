...
use `agent` unless `review_agents.reviewer` selects an independent reviewer;
every other pipeline invocation continues to use `agent`.
...
normal `agent` selection and fallback chain, with an omitted fixer retaining
[`review_fixer_agent`](#review_fixer_agent) and its launcher when configured;
other pipeline steps are unchanged. An explicit `review_agents.fixer` takes
precedence over both legacy fixer settings, so the unused legacy agent or
launcher does not need to be installed. Reviewer profiles never inherit the
legacy fixer launcher.
