...
You can send the existing Review step's fix turns to a dedicated agent while
keeping independent reviews and all other work on the default agent. The
[`review_fixer_agent` reference](/no-mistakes/reference/global-config/#review_fixer_agent)
owns setup, launcher, and session details.
...
By default, structured output is handled by appending the requested JSON schema to the prompt and validating the final assistant text with the common text fallback described above.
